...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('refresh-btn').addEventListener('click', function() {
        ...
        loadPage(active ? active.dataset.page : 'dashboard');
    });
});
...
async function renderChannels(container) {
    container.innerHTML = `
        ...
        <div class="table-responsive"><table class="table table-hover"><thead><tr><th>频道名</th><th>分类</th><th>延迟(ms)</th><th>固定</th><th>操作</th></tr></thead><tbody id="channel-tbody"><tr><td colspan="5" class="text-center">加载中...</td></tr></tbody></table></div>
    `;
    document.getElementById('apply-filter-btn').addEventListener('click', loadChannels);
    document.getElementById('search-input').addEventListener('keyup', function(e) { if(e.key === 'Enter') loadChannels(); });
    await loadChannels();
    ...
    async function loadChannels() {
        ...
        try {
            ...
            if(!channels.length) {
                tbody.innerHTML = '<tr><td colspan="5" class="text-center">暂无频道</td></tr>';
                return;
            }
            tbody.innerHTML = channels.map(ch => `
                <tr>
                    ...
                    <td><button class="btn btn-sm btn-outline-info view-quality" data-name="${ch.name}"><i class="fas fa-chart-line"></i></button></td>
                </tr>
            `).join('');
            document.querySelectorAll('.view-quality').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    setTimeout(() => {
                        ...
                        if(input) { input.value = name; document.getElementById('quality-search-btn').click(); }
                    }, 300);
                });
            });
...
async function renderFixed(container) {
    container.innerHTML = `
        ...
        <div class="table-responsive"><table class="table table-hover"><thead><tr><th>频道名</th><th>URL</th><th>操作</th></tr></thead><tbody id="fixed-tbody"><tr><td colspan="3" class="text-center">加载中...</td></tr></tbody></table></div>
    `;
        ...
        try {
            ...
            tbody.innerHTML = entries.map(([name, url]) => `
                <tr>
                    ...
                    <td><code>${url}</code></td>
                    <td><button class="btn btn-sm btn-danger delete-fixed" data-name="${name}"><i class="fas fa-trash-alt"></i></button></td>
                </tr>
            `).join('');
...
async function renderQuality(container) {
    container.innerHTML = `
        <div class="row mb-3">
            <div class="col-md-4"><input type="text" class="form-control" id="quality-channel-input" placeholder="输入频道名"></div>
            <div class="col-md-2"><button class="btn btn-primary" id="quality-search-btn">查看趋势</button></div>
            <div class="col-md-3">
        ...
        <div class="card"><div class="card-header" id="quality-chart-title">请选择频道查看延迟趋势</div><div class="card-body"><canvas id="quality-chart" height="300"></canvas></div></div>
    `;
            ...
            chartInstance = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        x: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#adb5bd', maxTicksLimit: 20 } }
                    }
                }
            });
        } catch(e) { console.error(e); }

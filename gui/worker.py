...
class CollectionWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 强制重新加载 src.config 以使环境变量生效
            if 'src.config' in sys.modules:
                importlib.reload(sys.modules['src.config'])
                self.log_signal.emit("🔄 已重新加载配置模块")

            # 验证环境变量是否生效
            from src.config import AUTONOMOUS_MODE
            self.log_signal.emit(f"🔧 AUTONOMOUS_MODE 值: {AUTONOMOUS_MODE}")

            # 如果仍然为 False，直接修改模块变量
            if not AUTONOMOUS_MODE:
                import src.config
                src.config.AUTONOMOUS_MODE = True
                self.log_signal.emit("🔧 已直接设置 AUTONOMOUS_MODE = True")
